...
def main(csv_path: str):
    ...
    # Per-size comparison bars: mean latency for UCX vs ZMQ
    means = (
        df.groupby(["size_bytes", "pattern"], as_index=False)["latency_usec"].mean()
    )
    pivot = means.pivot(index="size_bytes", columns="pattern", values="latency_usec").fillna(np.nan)
    patterns = [p for p in ["UCX", "ZMQ"] if p in pivot.columns]
    if not patterns:
        print("No UCX/ZMQ data found in CSV")
        sys.exit(1)
    ...
    plt.title("UCX vs ZMQ fan-in (mean over repeats)")
    plt.legend()
    plt.tight_layout()
    plt.savefig("compare_mean_ucx_vs_zmq.png", dpi=150, bbox_inches="tight")
    print("Saved compare_mean_ucx_vs_zmq.png")
    ...
    for size in sizes:
        ...
        plt.title(f"Latency by round {titles.get(int(size), str(int(size)))}")
        # Means
        mean_handles = []
        ...
        out_name = f"compare_rounds_{titles.get(int(size), str(int(size))).lower()}.png"
